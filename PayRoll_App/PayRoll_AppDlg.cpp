...
// PayRoll_AppDlg.cpp : implementation file
//

#include "stdafx.h"
#include "PayRoll_App.h"
#include "PayRoll_AppDlg.h"
...
CPayRoll_AppDlg::CPayRoll_AppDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(CPayRoll_AppDlg::IDD, pParent)
{
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void CPayRoll_AppDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
}

BEGIN_MESSAGE_MAP(CPayRoll_AppDlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(btnProcess, &CPayRoll_AppDlg::OnBnClickedbtnprocess)
END_MESSAGE_MAP()


// CPayRoll_AppDlg message handlers

BOOL CPayRoll_AppDlg::OnInitDialog()
{
...
void CPayRoll_AppDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
HCURSOR CPayRoll_AppDlg::OnQueryDragIcon()
{
...
void CPayRoll_AppDlg::OnBnClickedbtnprocess()
{
	// TODO: Add your control notification handler code here
	CString textInput;
	//Employee ID
	GetDlgItemText(txtEID, textInput);
	double dEmployeeId = _ttof(textInput);
	// Hourly Wage
	GetDlgItemText(txtHW, textInput);
	double dHourlyWage = _ttof(textInput);

	//First Week
	GetDlgItemText(txtFirstWeekMon, textInput);
	double dFirstMonday = _ttof(textInput);

	GetDlgItemText(txtFirstWeekTue, textInput);
	double dFirstTuesday = _ttof(textInput);

	GetDlgItemText(txtFirstWeekWed, textInput);
	double dFirstWednesday = _ttof(textInput);

	GetDlgItemText(txtFirstWeekThu, textInput);
	double dFirstThursday = _ttof(textInput);

	GetDlgItemText(txtFirstWeekFriday, textInput);
	double dFirstFriday = _ttof(textInput);

	GetDlgItemText(txtFirstWeekSat, textInput);
	double dFirstSaturday = _ttof(textInput);
	
	GetDlgItemText(txtFirstWeekSun, textInput);
	double dFirstSunday = _ttof(textInput);

	//Second Week
	GetDlgItemText(txtSecondWeekMon, textInput);
	double dSecondMonday = _ttof(textInput);

	GetDlgItemText(txtSecondWeekTue, textInput);
	double dSecondTuesday = _ttof(textInput);

	GetDlgItemText(txtSecondWeekWed, textInput);
	double dSecondWednesday = _ttof(textInput);

	GetDlgItemText(txtSecondWeekThu, textInput);
	double dSecondThursday = _ttof(textInput);

	GetDlgItemText(txtSecondWeekFri, textInput);
	double dSecondFriday = _ttof(textInput);

	GetDlgItemText(txtSecondWeekSat, textInput);
	double dSecondSaturday = _ttof(textInput);

	GetDlgItemText(txtSecondWeekSun, textInput);
